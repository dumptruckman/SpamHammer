...
abstract class ChatSpamHandler<T extends PlayerEvent> extends AbstractSpamHandler<T> {
    ...
    static final SimpleConfigEntry<Integer> REPEAT_LIMIT = new EntryBuilder<Integer>(Integer.class, "message.repeat.limit").def(2)
            .comment("If SpamHammer is set to block repeat messages, this is how many messages before they are "
                    + "considered repeats.").validator(new ZeroGreaterValidator()).build();
    static final SimpleConfigEntry<Integer> REPEAT_TIME_LIMIT = new EntryBuilder<Integer>(Integer.class, "message.repeat.time_limit").def(2)
            .comment("If SpamHammer is set to block repeat messages, this is how many messages before they are "
                    + "considered repeats.").validator(new ZeroGreaterValidator()).build();
